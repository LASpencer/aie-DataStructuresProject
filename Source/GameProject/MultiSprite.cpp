...
void MultiSprite::draw(aie::Renderer2D * renderer)
{
	...
	for (TexturePtr texture : m_textures) {
		renderer->drawSpriteTransformed3x3(texture->get(), &position->getGlobalTransform()[0][0], m_width, m_height, 0, m_xOrigin, m_yOrigin);//TODO fix bug where uvrect sprite is size of entire sheet
